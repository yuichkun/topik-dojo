...
### データベースの扱い
このプロジェクトではデータベースを**一切モックしません**。テスト環境では better-sqlite3 のインメモリDBを使用し、実際のデータベース操作をテストします。
...
**データベースクリーンアップについて**:
- データベースのクリーンアップは `jest.setup.js` で自動化済み
...
- ヘルパーがない場合は Drizzle の `db.insert()` で直接作成可能
