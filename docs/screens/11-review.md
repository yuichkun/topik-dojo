...
1. **覚えた選択**:
   - `mastery_level++`（最大5まで）
   - `interval_days = [1, 3, 7, 14, 30][mastery_level]`
   - `next_review_date = 今日 + interval_days`
   - `status = mastery_level >= 4 ? "mastered" : "learning"`

2. **覚えてない選択**:
   - `mastery_level = Math.max(0, mastery_level - 1)`
   - `interval_days = 1`
   - `next_review_date = 今日 + 1日`
   - `mistake_count++`
